...
-- Fixed and updated by naz (Discord: naz#6660)
...
local window_bkey = ui.Keys.insert
    ...
    if ui.BeginWindow("senseui.menu", 50, 50, 650, 700) then
        ...
        if ui.BeginTab("ragebot", ui.Icons.rage) then
            if ui.BeginGroup("rbot.aimbot", "Aimbot", 25, 25, 250, 650) then
                ...
                local doublefire = 0
                if Weapon ~= "zeus" then
                    gui.SetValue("rbot.accuracy.weapon."..Weapon..".doublefire", ui.Combo("Doubletap", {"Off", "Shift", "Rapid"}, gui.GetValue("rbot.accuracy.weapon."..Weapon..".doublefire") + 1) - 1)
            ...
            if ui.BeginGroup("rbot.misc", "Misc", 295, 460, 250, 215) then
                ...
                ui.Label("Auto Scope")
                gui.SetValue("rbot.aim.automation.scope", ui.Combo("scope", {"Off", "On - auto unzoom", "On - no unzoom"}, gui.GetValue("rbot.aim.automation.scope") + 1) - 1)
                
                --gui.SetValue("rbot.accuracy.movement.autopeek", ui.Checkbox("Quick Peek", gui.GetValue("rbot.accuracy.movement.autopeek")))

                if gui.GetValue("rbot.accuracy.movement.autopeekkey") == 0 then
                    ui.Label("Quick Peek")
                    gui.SetValue("rbot.accuracy.movement.autopeekkey", ui.Bind("peek", false, gui.GetValue("rbot.accuracy.movement.autopeekkey")))
                end
        ...
        if ui.BeginTab( "anti-aim", ui.Icons.antiaim) then
            ...
            if ui.BeginGroup("rbot.antiaim.extra", "Extra", 295, 460, 250, 215) then
                ...
                gui.SetValue("misc.fakelatency.key", ui.Bind("fakelatency.key", false, gui.GetValue("misc.fakelatency.key")))
                ...
                if gui.GetValue("rbot.master") then
                    ...
                    gui.SetValue("rbot.antiaim.extra.staticlegs", ui.Checkbox("Static legs", gui.GetValue("rbot.antiaim.extra.staticlegs"))) 
        ...
        if ui.BeginTab( "settings", ui.Icons.settings ) then
            ...
            if ui.BeginGroup("misc.movement", "Movement", 295, 250, 250, 425) then
                ...
                ui.Label("Mode")
                gui.SetValue("misc.strafe.mode", ui.Combo("misc.strafe.mode", {"Silent", "Normal", "Sideways", "W-Only", "Mouse"}, gui.GetValue("misc.strafe.mode") + 1) - 1)
        ...
        if ui.BeginTab( "playerlist", ui.Icons.playerlist ) then
            ...
            if ui.BeginGroup("misc.Credits", "Credits", 25, 200, 510, 150) then
                ...
                ui.Label("Fixed and updated by naz (Discord: naz#6660)", true)
...
callbacks.Register("Draw", function()
    ...
    local r, g, b, a = gui.GetValue("theme.header.bg")
    ...
    nxzTextSize = draw.GetTextSize("Sense")
    nxzAATextSize = draw.GetTextSize("SenseUI")
    betaTextSize = draw.GetTextSize(" beta")
    ...
    draw.TextShadow(15, 14, "Sense")
    draw.Color(r, g, b)
    draw.TextShadow(16 + nxzTextSize, 14, "UI")
    draw.Color(255, 255, 255)
